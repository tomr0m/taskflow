...
const TYPE_STYLE: Record<ItemType, CSSProperties> = {
...
function getEventStyle(item: Item): CSSProperties {
  const base = TYPE_STYLE[item.type];
  const urgentBorder: CSSProperties =
...
function toCalDates(item: Item): { start: Date; end: Date } {
...
function CalEventComponent({ event }: { event: CalEvent }) {
  const item = event.resource;
  return (
    <div className="flex items-center gap-1 overflow-hidden w-full px-0.5">
      <span className="shrink-0 opacity-80">
        <ItemTypeIcon type={item.type} size={9} />
      </span>
...
export const CalendarView = ({
  ...
  const handleEventDrop = useCallback(
    async ({ event, start }: { event: CalEvent; start: Date | string }) => {
      const item = event.resource;
    ...
    <div className="relative">
      ...
      >
        <DnDCalendar
          ...
          selectable={canCreateItems}
          components={{ event: CalEventComponent as any }}
          popup
          style={{ height: '100%' }}
